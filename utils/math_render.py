...
def render_math_formula(formula: str) -> str:
    ...
    """
    # URL-encode the formula (excluding spaces, which are stripped).
    # LaTeX special characters that are safe to leave unencoded in this URL context:
    # backslash, caret, braces, dollar, underscore, plus, asterisk, equals, pipe,
    # square brackets, parentheses, exclamation mark, and ampersand.
    # '<' and '>' are intentionally NOT in the safe set so they are encoded.
    formula_url = quote(formula.replace(' ', ''), safe=r'\\^{}$_+*=|[]()!&')
